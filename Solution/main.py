...
from solution.modules import Encoder, Decoder
from solution.custom_types import DaRnnNet, TrainData, TrainConfig
from solution.utils import numpy_to_tvar
from solution.constants import device
import time
...
raw_data = pd.read_csv(os.path.join("data", "APS_1a11.csv"), nrows=100 if debug else None)
#raw_data[['timestamp']]= time.mktime(time.strptime(raw_data[['timestamp']], "%Y-%m-%d %H:%M:%S"))

raw_data = raw_data[['arrival_delay','departure_delay']]
logger.info(f"Shape of data: {raw_data.shape}.\nMissing in data: {raw_data.isnull().sum().sum()}.")
targ_cols = ("arrival_delay",)
